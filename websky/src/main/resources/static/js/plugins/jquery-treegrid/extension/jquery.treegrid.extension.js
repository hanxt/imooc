			...
			$.each(options.columns, function(index, column) {
				...
				if(index==0&&column.field=='selectItem'){
                    ...
                    if (column.formatter) {
						...
						var innerColumn = column.field.split(".");//处理多级"."JSON取值
						var innerColumnValue = item[innerColumn[0]];
						if(innerColumn.length > 1){
							for (index in innerColumn) {
								if(index > 0){
									innerColumnValue = innerColumnValue[innerColumn[index]];
								}
							}
						}
                        td.text(innerColumnValue);
                    }
					tr.append(td);
				}
			});
